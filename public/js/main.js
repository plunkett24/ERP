$(document).ready(function () {
	...
	$('.delete-humanresources').on('click', function () {
		//make request to delete
		var id = $(this).data('id');
		var url = '/users/hr2/delete/' + id;
		if (confirm('Delete Employee?')) {
			$.ajax({
				...
				success: function (result) {
					...
					window.location.href = '/users/hr2';
				},
				error: function (err) {
					console.log(err);
				},
	...
	$('.edit-humanresources').on('click', function () {
		$('#edit-form-firstname').val($(this).data('firstname'));
		$('#edit-form-lastname').val($(this).data('lastname'));
		$('#edit-form-country').val($(this).data('country'));
		$('#edit-form-city').val($(this).data('city'));
		...
		$('#edit-form-phone').val($(this).data('phonenumber'));
		$('#edit-form-employee_position').val($(this).data('employee_position'));
		$('#edit-form-id').val($(this).data('id'));
	});
